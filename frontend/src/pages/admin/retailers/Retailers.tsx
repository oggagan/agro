...
import { useRetailerList, useUpdateRetailerStatus } from "@/hooks/useRetailers";
...
export default function Retailers() {
  ...
  const queryParams = {
    ...
    ...(companyTypeFilter !== "all" && { companyType: companyTypeFilter as "PVT_LTD" | "PROPRIETORSHIP" | "PARTNERSHIP" }),
  };

  const { data: response, isLoading } = useRetailerList(queryParams);
  const deleteUser = useDeleteUser();
  const updateStatus = useUpdateRetailerStatus();

  const retailers: Retailer[] = response?.data ?? [];
  const meta = response?.meta;

  const hasFilters = search || statusFilter !== "all" || companyTypeFilter !== "all";
  ...
  const handleStatusToggle = (ret: Retailer) => {
    const newStatus: UserStatus = ret.user.status === "ACTIVE" ? "INACTIVE" : "ACTIVE";
    updateStatus.mutate({ id: ret.id, status: newStatus });
  };

  return (
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <h1 className="text-xl font-semibold tracking-tight text-foreground">Retailers</h1>
        <Button
          className="bg-gradient-primary h-10"
          onClick={() => navigate("/dashboard/admin/retailers/add")}
        >
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    ...
                    <TableHead className="h-10 text-xs font-medium text-muted-foreground">Licenses</TableHead>
                  ...
                  {retailers.length === 0 ? (
                    ...
                    retailers.map((ret) => (
                      <TableRow key={ret.id} className="hover:bg-muted/40">
                        <TableCell className="font-medium py-3">{ret.companyName}</TableCell>
                        <TableCell className="py-3 text-muted-foreground">{ret.user.name}</TableCell>
                        <TableCell className="py-3 font-mono text-sm">{ret.user.phone ?? "—"}</TableCell>
                        <TableCell className="py-3 font-mono text-sm text-muted-foreground">
                          {ret.gstNumber || "—"}
                        </TableCell>
                        <TableCell className="py-3 text-muted-foreground text-sm">
                          {ret.companyType.replace("_", " ").toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase())}
                        </TableCell>
                        <TableCell className="py-3">
                          <div className="flex flex-wrap gap-1">
                            {(ret.licenses ?? []).length === 0 ? (
                              <span className="text-muted-foreground text-xs">—</span>
                            ) : (
                              (ret.licenses ?? []).map((lic) => (
                                <Badge key={lic.id} variant="secondary" className="text-xs font-normal">
                                  {lic.category}
                                </Badge>
                              ))
                            )}
                        ...
                        <TableCell className="py-3">
                          <Badge
                            variant="outline"
                            className={`cursor-pointer text-xs font-medium ${statusStyles[ret.user.status] ?? "bg-muted/80 text-muted-foreground"}`}
                            onClick={() => handleStatusToggle(ret)}
                          >
                        ...
                        <TableCell className="py-3 text-right">
                          <div className="flex justify-end gap-1">
                            <Button
                              ...
                              size="icon"
                              className="h-8 w-8"
                              onClick={() => navigate(`/dashboard/admin/retailers/edit/${ret.id}`)}
                              title="Edit"
                            >
