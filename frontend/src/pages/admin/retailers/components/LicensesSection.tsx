...
import {
  ...
  useUploadRetailerLicenseDocuments,
...
import type { RetailerLicense } from "@/types/retailer";
import type { LicenseCategory } from "@/types/retailer";
...
function filterDocs(lic: RetailerLicense, docType: string) {
  return (lic.documents ?? []).filter((d) => d.docType === docType);
}

export default function LicensesSection({ retailerId, licenses }: LicensesSectionProps) {
  const addLicense = useAddRetailerLicense();
  const updateLicense = useUpdateRetailerLicense();
  const uploadDocs = useUploadRetailerLicenseDocuments();
  ...
  const startEdit = (lic: RetailerLicense) => {
        ...
        {licenses.map((lic) => (
          ...
          >
            ...
            <InlineDocUpload
              ...
              onUpload={(files) => uploadDocs.mutate({ retailerId, licId: lic.id, files, docType: "license" })}
