...
import {
  useAddDistributorDirector,
  useUpdateDistributorDirector,
  useDeleteDistributorDirector,
  useUploadDistributorDirectorDocuments,
...
import type { DistributorDirector } from "@/types/distributor";
...
export default function DirectorsSection({
  ...
  const addDirector = useAddDistributorDirector();
  const updateDirector = useUpdateDistributorDirector();
  const deleteDirector = useDeleteDistributorDirector();
...
function DirectorRow({
  ...
  director: DistributorDirector;
  index: number;
  distributorId: string;
  personLabel: string;
  onUpdate: ReturnType<typeof useUpdateDistributorDirector>;
  onDelete: ReturnType<typeof useDeleteDistributorDirector>;
}) {
  const uploadDirDocs = useUploadDistributorDirectorDocuments();
      ...
      <div className="border-t pt-3 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ distributorId, dirId: director.id, files, docType: "aadhaar_front" })
            }
          ...
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ distributorId, dirId: director.id, files, docType: "aadhaar_back" })
            }
            onDelete={(docId) => deleteDoc.mutate({ distributorId, docId })}
            uploading={uploadDirDocs.isPending}
            deleting={deleteDoc.isPending}
          ...
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ distributorId, dirId: director.id, files, docType: "pan_front" })
            }
            onDelete={(docId) => deleteDoc.mutate({ distributorId, docId })}
            uploading={uploadDirDocs.isPending}
            deleting={deleteDoc.isPending}
        ...
        {editing ? (
          ...
          <>
            ...
            <Button
              ...
              onClick={() => onDelete.mutate({ distributorId, dirId: director.id })}
