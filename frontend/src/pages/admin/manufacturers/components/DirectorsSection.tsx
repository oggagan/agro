...
import type { Director, CompanyType, ManufacturerDocument } from "@/types/manufacturer";
...
export default function DirectorsSection({ manufacturerId, directors, companyType }: DirectorsSectionProps) {
  ...
  const [newDirector, setNewDirector] = useState({
    name: "", designation: "", phone: "", email: "", aadhaarNumber: "", panNumber: "",
  });
...
function DirectorRow({
  ...
  manufacturerId: string;
  personLabel: string;
  onUpdate: ReturnType<typeof useUpdateDirector>;
  onDelete: ReturnType<typeof useDeleteDirector>;
}) {
  const uploadDirDocs = useUploadDirectorDocuments();
      ...
      <div className="border-t pt-3 space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ mfgId: manufacturerId, dirId: director.id, files, docType: "aadhaar_front" })
            }
            onDelete={(docId) => deleteDoc.mutate({ mfgId: manufacturerId, docId })}
          ...
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ mfgId: manufacturerId, dirId: director.id, files, docType: "aadhaar_back" })
            }
            onDelete={(docId) => deleteDoc.mutate({ mfgId: manufacturerId, docId })}
            uploading={uploadDirDocs.isPending}
            deleting={deleteDoc.isPending}
          ...
          <InlineDocUpload
            ...
            onUpload={(files) =>
              uploadDirDocs.mutate({ mfgId: manufacturerId, dirId: director.id, files, docType: "pan_front" })
            }
            onDelete={(docId) => deleteDoc.mutate({ mfgId: manufacturerId, docId })}
            uploading={uploadDirDocs.isPending}
            deleting={deleteDoc.isPending}
        ...
        {editing ? (
          ...
          <>
            ...
            <Button
              ...
              onClick={() => onDelete.mutate({ mfgId: manufacturerId, dirId: director.id })}
