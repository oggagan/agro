...
import {
  useUpsertAuthorizedPerson,
  useAddAuthorizedPerson,
  useUpdateAuthorizedPerson,
  useUploadAuthorizedPersonDocuments,
  useDeleteDocument,
...
interface AuthorizedPersonFormProps {
  ...
  apId?: string;
  existing?: AuthorizedPerson | null;
  onClose?: () => void;
}
...
export default function AuthorizedPersonForm({
  ...
  const upsert = useUpsertAuthorizedPerson();
  const addAp = useAddAuthorizedPerson();
  const updateAp = useUpdateAuthorizedPerson();
  const uploadApDocs = useUploadAuthorizedPersonDocuments();
  ...
  const pending = addAp.isPending || updateAp.isPending || upsert.isPending;
        ...
        <div className="border-t pt-4 space-y-3">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <InlineDocUpload
              ...
              onUpload={(files) =>
                uploadApDocs.mutate({ mfgId: manufacturerId, apId: effectiveApId, files, docType: "aadhaar_front" })
              }
            ...
            <InlineDocUpload
              ...
              onUpload={(files) =>
                uploadApDocs.mutate({ mfgId: manufacturerId, apId: effectiveApId, files, docType: "aadhaar_back" })
              }
              onDelete={(docId) => deleteDoc.mutate({ mfgId: manufacturerId, docId })}
              uploading={uploadApDocs.isPending}
              deleting={deleteDoc.isPending}
            ...
            <InlineDocUpload
              ...
              onUpload={(files) =>
                uploadApDocs.mutate({ mfgId: manufacturerId, apId: effectiveApId, files, docType: "declaration" })
              }
              onDelete={(docId) => deleteDoc.mutate({ mfgId: manufacturerId, docId })}
              uploading={uploadApDocs.isPending}
              deleting={deleteDoc.isPending}
